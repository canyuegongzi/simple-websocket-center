...
export class OrganizationService {
   ...
   */
    async creatOrganization(createOrganizationDto: CreateOrganizationDto): Promise<any> {
      try {
        const currentOrgan: Organization = await this.organizationRepository.findOne({id: createOrganizationDto.parentId});
        if (!currentOrgan) {
    ...
    public async getList(query: QueryOrganizationDto) {
        try {
            const res = await this.organizationRepository
                ...
                .orWhere('o.name like :name', { name: `${query.name.length > 2 ? `%${query.name}%` : '%%'}`})
                .orderBy('o.name', 'ASC')
